...
describe("TC-SETTLE-01: destination charge フロー", () => {
  ...
  it("settle 後: organizer・artist それぞれに source_transaction Transfer が作成される", async () => {
    ...
    // →照合→送金、の順を強制するため）。reconciled_atはinsertTransactionが
    // デフォルトで設定済みなので、ここでは開催承認相当の実キャプチャのみ行う
    // （stripe_net_actualは未設定のままとなり、net_amountの推定値ベースで
    // 分配される既存の期待値と一致させる）。
...
describe("TC-PAY-05: SavedCard off_session オーソリ → settle で1円単位3者分配", () => {
  ...
  it("開催承認（キャプチャ）後、送金ロジックが1円単位で3者分配する", async () => {
    mockAdminAuth();
    await stripe.paymentIntents.capture(offSessionPiId);
    const req = new Request("http://localhost", { method: "POST" });
...
// ── TC-SETTLE-07: PayPay 即時キャプチャ済み PI のセトルメント ────────────
describe("TC-SETTLE-07: PayPay 即時キャプチャ済み PI（succeeded）のセトルメント", () => {
  /**
   * PayPay は capture_method: "automatic" で即時キャプチャされるため、
   * PI は購入時点で succeeded 状態になる。
   * settle ロジックがキャプチャをスキップし、source_transaction で
   * 3者（organizer / artist / agent）に1円単位で正確に分配できることを確認する。
   */
  const GROSS = 10_000;
  ...
  beforeAll(async () => {
    ...
    pay07EventId = await insertEvent({
      organizerProfileId,
      agentId: agentProfileId,
      title: "TC-SETTLE-07 PayPay決済",
    });
    const qrConfigId = await insertQrConfig({
      eventId: pay07EventId,
      creatorProfileId: organizerProfileId,
      recipientProfileId: artistProfileId,
    });
    cleanup.eventIds.push(pay07EventId);
    cleanup.qrConfigIds.push(qrConfigId);
  ...
  it("送金ロジックが succeeded PI で source_transaction で3者分配する", async () => {
    ...
    // settle_transfers の検証（1円の狂いもなく3者分配）
    const { data: transfers } = await testAdmin
      .from("settle_transfers")
      .select("profile_id, amount, stripe_transfer_id")
      .eq("event_id", pay07EventId);
    ...
    // 合計額の検証
    const total = (transfers ?? []).reduce((s: number, t: any) => s + t.amount, 0);
...
describe("TC-SETTLE-08: 未照合の決済が残っていると送金が中断される", () => {
  ...
  beforeAll(async () => {
    ...
    txId = await insertTransaction({
      ...
      platformFee: Math.floor(gross * 0.10),
      stripePaymentIntentId: pi.id,
      reconciled: false, // insertTransactionはデフォルトで即reconciled_at済みにするため、未照合を明示する
    });
    cleanup.transactionIds.push(txId);
