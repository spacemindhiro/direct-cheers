 ...
 * - Connect発行済み・capability完了 → on_behalf_of設定・statement_descriptor_suffixはイベント名
...
describe("TC-ENT-SD-01: オーガナイザーがConnect未発行 → ブロックせずsuffixのみ付与", () => {
  ...
  it("200で決済リンクが返り、on_behalf_ofは省略・suffixはイベント名になる", async () => {
    ...
    expect(pid?.statement_descriptor_suffix).toBe("SPACE BBQ FESTIVAL");
  });
});
...
describe("TC-ENT-SD-02: Connect発行済み・capability完了 → on_behalf_of・suffixともに設定される", () => {
  ...
  it("イベント名が漢字のみの場合、statement_descriptor_suffixは省略される（ASCII化不能）", async () => {
    const req = new Request("http://localhost/api/entrance/reserve", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
