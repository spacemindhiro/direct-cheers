  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between">
        ...
        {alertCount > 0 && (
          <div className="flex items-center gap-2 bg-red-950/40 border border-red-500/30 px-4 py-2 rounded-2xl">
            ...
            <span className="text-red-300 text-sm font-black">要確認 {alertCount}件</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "今日の処理", value: (reports ?? []).filter(r => r.process_date === today).length + "件", sub: "本日実行済みバッチ", color: "text-white" },
          { label: "要確認", value: alertCount + "件", sub: "未回収・エラーあり", color: alertCount > 0 ? "text-red-400" : "text-slate-500" },
          { label: "未対応明細", value: (initialDetails ?? []).filter((d: any) => d.action_status === "未対応").length + "件", sub: "対応待ちリスク", color: "text-amber-400" },
          { label: "直近30件", value: (reports ?? []).length + "件", sub: "レポート総数", color: "text-slate-300" },
        ].map(stat => (
      ...
      <BatchReportClient reports={reports ?? []} initialDetails={initialDetails ?? []} />
    </div>
  );
}
