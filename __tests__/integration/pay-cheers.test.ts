...
describe("TC-PAY-05: statement_descriptor_suffix（宛先名義による動的切替）", () => {
  ...
  beforeAll(async () => {
    ...
    qrArtistConfigId = await insertQrConfig({ eventId, creatorProfileId: organizerProfileId, recipientProfileId: artistProfileId, productId });
    ...
    qrKanjiConfigId = await insertQrConfig({ eventId, creatorProfileId: organizerProfileId, recipientProfileId: kanjiOnlyArtistProfileId, productId });
