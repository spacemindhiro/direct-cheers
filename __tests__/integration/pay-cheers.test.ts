...
describe("TC-PAY-05: statement_descriptor_suffix（宛先名義による動的切替）", () => {
  ...
  beforeAll(async () => {
    ...
    // organizer文脈のsuffixはイベント名になる（主催者名はベース表記側に出るため重複回避）。
    // 検証しやすいよう英語タイトルの専用イベントを使う。
    asciiEventId = await insertEvent({ organizerProfileId, title: "SUMMER FEST" });
    cleanup.eventIds.push(asciiEventId);
    ...
    qrOrganizerConfigId = await insertQrConfig({ eventId: asciiEventId, creatorProfileId: organizerProfileId, recipientProfileId: organizerProfileId });
  ...
  it("recipient_name_context='organizer' → suffixはorganizer_nameではなくイベント名から生成される（SUMMER FEST）", async () => {
    const req = new Request("http://localhost/api/pay/cheers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    expect(res.status).toBe(200);
    const pid = captured.sessionCreateParams?.payment_intent_data;
    // organizer_name="SPACE BBQ"は無視され、イベント名が使われる
    // （主催者名は account-level prefix 側に出るため、suffixで繰り返さない）
    expect(pid?.statement_descriptor_suffix).toBe("SUMMER FEST");
    expect(pid?.statement_descriptor_suffix).not.toBe("SPACE BBQ");
  });
