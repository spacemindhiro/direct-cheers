...
describe("TC-ENT-COMPLETE-C: タイプA SetupIntent 完了経路（タイプCは当日決済専用のため対象外）", () => {
  ...
  beforeAll(async () => {
    ...
    productCId = await insertProduct({ eventId, paymentType: "C", name: "TypeC 当日決済テスト券" });
    ...
    // タイプCは本来 /reserve で予約自体が作れない（400で拒否される）が、
    // /complete 単体が異常系として正しく拒否することも確認するため、
    // reservation行だけを直接DBに作って到達させる。
  ...
  it("TC-ENT-COMPLETE-C-02: タイプC + payment_method_id → 400（当日決済専用のためこの経路は不可）", async () => {
    const req = new Request("http://localhost/api/entrance/complete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
