...
// ── TC-TAX-07: PayPay 手数料への消費税（10%）上乗せ検証 ──────────────────
describe("TC-TAX-07: PayPay 手数料の消費税上乗せ（法的根拠の明示）", () => {
  /**
   * Stripe カード手数料（3.6%）は非課税（金融取引として消費税法上の非課税取引）。
   * PayPay 決済手数料（3.98%）は課税取引として消費税（10%）が上乗せされる。
   * 実質手数料 = 3.98% × 1.1 = 4.378%
   * これを fee-config の paypay_rate に反映済みであることを確認する。
   */
  const PAYPAY_BASE_RATE = 0.0398;      // PayPay 公称手数料（税抜）
  const CONSUMPTION_TAX_RATE = 1.1;     // 消費税 10% 上乗せ係数
  const PAYPAY_EFFECTIVE_RATE = 0.04378; // 実質手数料（tax 込み）

  it("3.98% × 1.1 = 4.378% の消費税上乗せ計算が正しい", () => {
    const computed = Math.round(PAYPAY_BASE_RATE * CONSUMPTION_TAX_RATE * 100000) / 100000;
    expect(computed).toBe(PAYPAY_EFFECTIVE_RATE);
  });

  it("PayPay 手数料（4.378%）はカード手数料（3.96%）より高い", () => {
    expect(PAYPAY_EFFECTIVE_RATE).toBeGreaterThan(DEFAULT_STRIPE_RATE);
  });

  it("fee-config のデフォルト paypay_rate が消費税込み実質レートと一致する", async () => {
    const config = await getFeeConfig();
    expect(config.paypay_rate).toBe(PAYPAY_EFFECTIVE_RATE);
  });

  it("fee-config の paypay_net_rate が 1 - 0.04378 - 0.10 = 0.85622 と一致する", async () => {
    const config = await getFeeConfig();
    expect(config.paypay_net_rate).toBe(0.85622);
  });

  const paypayCases = [1_000, 3_333, 5_000, 10_000, 30_000];
  paypayCases.forEach((gross) => {
    it(`¥${gross.toLocaleString()} PayPay: 恒等式（net + fee ≤ gross、差 ≤ 2円）が成立する`, () => {
      const stripeFee = Math.floor(gross * PAYPAY_EFFECTIVE_RATE);
      const platformFee = Math.floor(gross * DEFAULT_PLATFORM_RATE);
      const net = gross - stripeFee - platformFee;
      expect(net).toBeGreaterThan(0);
      expect(net + stripeFee + platformFee).toBe(gross);
    });
  });

  it("PayPay の消費税分（=手数料差額）は grossの約0.418%", () => {
    const gross = 10_000;
    const taxOnFee = Math.floor(gross * PAYPAY_EFFECTIVE_RATE) - Math.floor(gross * PAYPAY_BASE_RATE);
    // 消費税分 ≈ 10000 × 0.0398 × 0.1 = 39.8 → 39 or 40 円
    expect(taxOnFee).toBeGreaterThanOrEqual(39);
    expect(taxOnFee).toBeLessThanOrEqual(40);
  });
});
