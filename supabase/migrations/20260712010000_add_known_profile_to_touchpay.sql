-- タッチ決済（Case④）で、同じcard_fingerprintが既に別のticketで実在アカウントに
-- 紐付いている（＝リピーター）と判明した場合、新しいticket/transactionもその場で
-- 同じprofileに紐付けられるようにする。p_known_profile_idが渡されればsender_profile_id
-- に即座にセットする（NULLのままにして次のサインアップQR頼みにしない）。
