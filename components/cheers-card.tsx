...
/** マウス/傾きでキラリと光るカードエフェクト */
function useKiraEffect() {
  ...
  useEffect(() => {
    const handler = (e: DeviceOrientationEvent) => {
      const x = Math.max(-1, Math.min(1, (e.beta ?? 0) / 25));
      const y = Math.max(-1, Math.min(1, (e.gamma ?? 0) / 25));
      setTilt({ x: x * -10, y: y * 10 });
      setGlare({ x: (y + 1) * 50, y: (x + 1) * 50 });
      setActive(true);
    };
  ...
  const cardStyle: React.CSSProperties = {
    ...
    transition: active ? "transform 0.08s ease-out" : "transform 0.4s ease-out",
    willChange: "transform",
  };

  const glareStyle: React.CSSProperties = {
    background: `radial-gradient(ellipse at ${glare.x}% ${glare.y}%, rgba(255,255,255,${active ? 0.18 : 0.05}) 0%, transparent 65%)`,
    transition: active ? "background 0.08s" : "background 0.4s",
  };
  ...
  const holoStyle: React.CSSProperties = {
    background: `linear-gradient(
      ...
      rgba(236,72,153,0.12) 80%,
      rgba(99,102,241,0.12) 100%
    )`,
    opacity: active ? 0.8 : 0.35,
    transition: "opacity 0.3s, background 0.12s",
  };

  return { cardRef, cardStyle, glareStyle, holoStyle, handleMouseMove, handleMouseLeave };
}
...
export function CheersCard({
  ...
  const { cardRef, cardStyle, glareStyle, holoStyle, handleMouseMove, handleMouseLeave } = useKiraEffect();
  ...
  return (
    <div className="relative select-none" style={{ perspective: "1200px" }}>
      ...
      >
        {/* ──────────── 表面 ──────────── */}
        <div
          className="relative overflow-hidden rounded-[1.75rem] border shadow-[0_8px_40px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.06)]"
          style={{
            ...cardStyle,
        ...
        >
          {/* ── ホログラムシート（常時） ── */}
          <div className="absolute inset-0 rounded-[1.75rem] pointer-events-none z-10 mix-blend-screen" style={holoStyle} />

          {/* ── グレアスポット（マウス追従） ── */}
          <div className="absolute inset-0 rounded-[1.75rem] pointer-events-none z-20" style={glareStyle} />
