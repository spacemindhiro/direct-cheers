...
describe("TC-POST-PAY-02: 照合差分検知 — StripeとDBの金額不一致を検知する", () => {
  ...
  beforeAll(async () => {
    ...
    mismatchTxId = await insertTransaction({
      ...
      netAmount: wrongGross - Math.ceil(wrongGross * 0.0396) - Math.floor(wrongGross * 0.1),
      stripeFee: Math.ceil(wrongGross * 0.0396),
...
describe("TC-POST-PAY-03: サブエージェント含む多者分配 — 合計が1円単位で一致する", () => {
  ...
  const MULTI_STRIPE_FEE = Math.ceil(MULTI_GROSS * 0.0396);
  ...
  it("4者分配（agent+org+artist1+artist2）の Transfer 合計が net - platform_fee に一致する", async () => {
    ...
    expect(maxExpected - total).toBeLessThan(10); // 端数誤差は 10円未満
