  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <ul className="list-disc pl-4 space-y-1">
          <li>売上は入金から14日後に出金可能になります。振込手数料として¥500が差し引かれます</li>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <Faq
            q="出金はいつからできますか？"
            a="入金から14日後に出金可能になります（振込手数料¥500が差し引かれます）。「出金管理」で「◯月◯日以降に出金可能」と表示されます。"
          />
          <Faq
...
function OrganizerGuide() {
  return (
    <div className="space-y-4">
      ...
      >
        <ul className="list-disc pl-4 space-y-1">
          <li><b>表示用タブレット（子機）を用意できない場合は、印刷したQRコードを会場に掲示するだけで運用できます。</b>QR詳細画面の「印刷する」から会場掲示用の高解像度QRを出力し、テーブルや壁に貼っておけば、お客様がご自身のスマートフォンで読み取って直接決済できます。子機や親機パネルの設置は不要です</li>
          <li>スタッフが対応できる場合は「入場スキャナを起動」からカメラでお客様のチケットQRを読み取り、「入場OK」「入場済みです」「無効なチケット」等を即座に判定できます</li>
          <li>「対面タッチ決済」（Touch Pay、タッチ決済用アプリが必要）は、Bluetoothカードリーダーを接続し、対象商品（QR作成時に「対面タッチ決済を許可する」をONにしたもの）をその場で決済できます。決済完了後、初めてのお客様には子機のQRコードを読み取ってもらいます</li>
          ...
          <li>誤操作・誤販売があった場合は、イベント詳細の「売上・決済」タブ（決済ログ）から自分のイベントの決済を取り消せます。カード決済はオーソリ取消（資金移動なし）、PayPayなど決済済みのものは返金となり決済手数料はオーガナイザー負担です。この取消はイベントが「精算済み」になる前のみ可能で、精算後の返金は運営（admin）のみが対応します</li>
        </ul>
      </Section>
      ...
      <Section
        ...
        subtitle="Payout"
      >
        <p>「出金管理」で「出金可能」「保留中」「凍結中」の残高を確認できます。売上は入金から14日後に出金可能になり、振込手数料¥500が差し引かれます。</p>
      ...
      >
        ...
        <ul className="list-disc pl-4 space-y-1">
          ...
          <li>PayPayなど既に決済が完了しているものは自動では取り消されないため、イベント詳細の「売上・決済」タブ（決済ログ）から個別に取消（返金）が必要です</li>
        </ul>
      </Section>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <Faq
            ...
            a="はい、できます。精算前のイベントであれば、イベント詳細の「売上・決済」タブ（決済ログ）から自分のイベントの決済を取り消せます（カード決済はオーソリ取消、PayPayなどは返金）。イベントが「精算済み」になった後の返金のみ、運営（admin）が対応します。"
          />
          <Faq
            q="前売り分はどうなりますか？"
            a="ほとんどのカード決済は「オーソリのみ」で保留されているため、イベント中止が承認されると自動的に取り消され、お客様に負担はかかりません。PayPayなど即時決済済みのものは自動では取り消されないため、決済ログから個別に取消（返金）してください。"
          />
          <Faq
