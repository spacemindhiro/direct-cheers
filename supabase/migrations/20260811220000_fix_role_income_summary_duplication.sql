-- get_role_income_summary の再デプロイ用マイグレーション
--
-- 20260811190000_add_role_income_summary.sql をマージ後に2回修正したが、
-- Supabaseのマイグレーション適用はファイル名(バージョン番号)単位で管理されており、
-- 同じファイルを後から編集しても既に適用済みのバージョンとして本番でスキップされ、
-- 本番のRPC定義が古いまま（agent/organizer二重計上バグを含む状態）だった。
-- 新しいバージョン番号のマイグレーションとして、現在のCREATE OR REPLACE FUNCTION
-- 定義を再度明示的にpushする。
