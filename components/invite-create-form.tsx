        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8 space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-slate-600 leading-relaxed">
              まだ登録していない相手を、上で選んだ「{ROLE_LABELS[targetRole] ?? targetRole}」ロールで直接招待します。同じ相手への既存の招待は自動的に無効化されます。
            </p>
          </div>

          {emailError && (
            <p className="text-sm text-red-400 font-bold">{emailError}</p>
          )}

          <button
            type="submit"
            disabled={isEmailPending || !targetEmail.trim()}
            className="w-full h-14 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isEmailPending ? (
              <Loader2 size={20} className="animate-spin" />
            ) : (
              <>メールアドレスで招待を送信 <ArrowRight size={18} /></>
            )}
          </button>
        </div>
      </form>
