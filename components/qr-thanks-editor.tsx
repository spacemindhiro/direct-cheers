...
export function QRThanksEditor({ qrConfigId }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/qr/${qrConfigId}/thanks/manage`)
      .then((r) => r.json())
      .then((data) => {
        if (data.thanks) {
          setState({
  ...
  const handleSave = async (publish?: boolean) => {
    ...
    setSaved(false);
    try {
      const res = await fetch(`/api/qr/${qrConfigId}/thanks`, {
        method: "POST",
      ...
      <div className="px-6 pt-6 pb-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">このQRで支払った人だけに届く特別なお返し</p>
          </div>
        </div>
        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
          ...
          {isPublished ? <><Eye size={10} /> 公開中</> : <><EyeOff size={10} /> 下書き</>}
        </div>
      </div>
      ...
      <div className="px-6 pb-6 space-y-4">
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="ファンへのメッセージ（例：ありがとう！次のライブでまた会おうね）"
            rows={3}
