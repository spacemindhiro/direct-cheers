...
import { GET as eligibleEditorGET, PATCH as eligibleEditorPATCH } from "@/app/api/qr/[qrConfigId]/welcome-cheer-eligible/route";
...
describe("TC-WC-E: QR編集画面からウェルカムチア対象演者を後から追加・削除できる", () => {
  let entranceProductId: string;
  let entranceQrConfigId: string;
  let defaultWcProductId: string;
  ...
  beforeAll(async () => {
    ...
    const data = await res.json();
    entranceProductId = data.product_id;
    entranceQrConfigId = data.qr_config_id;
    defaultWcProductId = data.welcome_cheer_product_id;
    cleanup.productIds.push(entranceProductId, defaultWcProductId);
    cleanup.qrConfigIds.push(entranceQrConfigId);
    const { data: wcQr } = await testAdmin
      .from("qr_configs").select("qr_config_id").eq("product_id", defaultWcProductId).single();
    cleanup.qrConfigIds.push(wcQr!.qr_config_id);

    // 演者本人の既存ワンプライスチアQR（QR作成時には選ばなかった想定）
    artistProductId = await insertProduct({
      eventId, type: "standard", name: "後から追加する演者チア", minAmount: 500, maxAmount: 500, artistId: artistProfileId,
    });
    const artistQrConfigId = await insertQrConfig({
  ...
  it("TC-WC-E-01: GET — 初期状態では候補は未選択、既存の¥500チアQRが候補一覧に出る", async () => {
    const res = await eligibleEditorGET(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`),
      { params: Promise.resolve({ qrConfigId: entranceQrConfigId }) },
    );
    const data = await res.json();
    expect(res.status).toBe(200);
    expect(data.welcome_cheer_amount).toBe(500);
    expect(data.eligible_product_ids).toEqual([]);
    expect(data.candidates.some((c: any) => c.product_id === artistProductId)).toBe(true);
  });

  it("TC-WC-E-02: PATCH — 演者を追加すると候補に反映される", async () => {
    const res = await eligibleEditorPATCH(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_ids: [artistProductId] }),
      }),
      { params: Promise.resolve({ qrConfigId: entranceQrConfigId }) },
    );
    expect(res.status).toBe(200);

    // デフォルト受取先（主催者）の行は編集対象外として常に残り続けるので、
    // それを除いた「主催者が明示的に足した候補」だけを見る。
    const { data: rows } = await testAdmin
      .from("welcome_cheer_eligible_products")
      .select("cheer_product_id")
      .eq("entrance_product_id", entranceProductId)
      .neq("cheer_product_id", defaultWcProductId);
    expect(rows?.map((r) => r.cheer_product_id)).toEqual([artistProductId]);
  });

  it("TC-WC-E-03: PATCH — 空配列で保存すると（デフォルト以外は）全て削除される", async () => {
    const res = await eligibleEditorPATCH(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    // デフォルト受取先の行自体は消えていない
    const { data: defaultRow } = await testAdmin
      .from("welcome_cheer_eligible_products")
      .select("cheer_product_id")
      .eq("entrance_product_id", entranceProductId)
      .eq("cheer_product_id", defaultWcProductId)
      .maybeSingle();
    expect(defaultRow).not.toBeNull();
  });

  it("TC-WC-E-04: PATCH — 金額不一致の商品は400で拒否される", async () => {
    const wrongPriceProductId = await insertProduct({
      eventId, type: "standard", name: "金額違い（編集テスト）", minAmount: 999, maxAmount: 999, artistId: artistProfileId,
    });
    ...
    const res = await eligibleEditorPATCH(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_ids: [wrongPriceProductId] }),
      }),
      { params: Promise.resolve({ qrConfigId: entranceQrConfigId }) },
    );
    expect(res.status).toBe(400);
  ...
  it("TC-WC-E-05: 他QR削除等で商品が対象条件を満たさなくなった不整合行が紛れ込んでいても、GETは隠しPATCHは自動で除去し保存を妨げない", async () => {
    // 過去のバグ・移行期に紛れ込みうる状況を再現：welcome_cheer_eligible_products に
    // 対象条件を満たさない（=候補一覧に出ない）商品への参照行が直接残っている状態。
    const strayProductId = await insertProduct({
      eventId, type: "standard", name: "不整合再現用（削除済み想定）", minAmount: 500, maxAmount: 500, artistId: artistProfileId,
    });
    cleanup.productIds.push(strayProductId);
    await testAdmin.from("welcome_cheer_eligible_products").insert({
      entrance_product_id: entranceProductId,
      cheer_product_id: strayProductId,
    });
    // 対象条件を満たさなくする（他QR削除に伴う商品削除を模す）
    await testAdmin.from("products").update({ deleted_at: new Date().toISOString() }).eq("product_id", strayProductId);

    // GET: 不整合行は候補一覧はもちろん、選択済み一覧にも出てこない
    const getRes = await eligibleEditorGET(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`),
      { params: Promise.resolve({ qrConfigId: entranceQrConfigId }) },
    );
    const getData = await getRes.json();
    expect(getData.eligible_product_ids).not.toContain(strayProductId);
    expect(getData.candidates.some((c: any) => c.product_id === strayProductId)).toBe(false);

    // PATCH: 演者を1人選んで保存 — 不整合行のせいで400にならず、かつ不整合行は自動で消える
    const patchRes = await eligibleEditorPATCH(
      new Request(`http://localhost/api/qr/${entranceQrConfigId}/welcome-cheer-eligible`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_ids: [artistProductId] }),
    ...
    const { data: rows } = await testAdmin
      ...
      .eq("entrance_product_id", entranceProductId)
      .neq("cheer_product_id", defaultWcProductId);
    expect(rows?.map((r) => r.cheer_product_id).sort()).toEqual([artistProductId]);
  });
});
