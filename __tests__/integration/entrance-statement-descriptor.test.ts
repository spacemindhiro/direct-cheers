 ...
 * - Connect発行済み・capability完了 → on_behalf_of設定。statement_descriptor_suffixは
 *   常に送らない（入場券のMoRは常にオーガナイザーで、ベース表記（DC-主催者名）に
 *   既に主催者名が出ているため、suffixで同じ名前を重複させると漢字17文字・カナ22文字の
 *   枠内で文字数があふれ、単語の途中で切れた意味不明な表記になる）
...
describe("TC-ENT-SD-01: オーガナイザーがConnect未発行 → ブロックせずsuffixのみ付与", () => {
  ...
  it("200で決済リンクが返り、on_behalf_ofは省略・suffixは送らない（ベース表記に主催者名が出るため重複させない）", async () => {
...
describe("TC-ENT-SD-02: Connect発行済み・capability完了 → on_behalf_of・suffixともに設定される", () => {
  ...
  it("入場券はsuffixを送らない（主催者名が漢字のみでも、そもそも主催者名義はsuffix自体を省略する）", async () => {
    const req = new Request("http://localhost/api/entrance/reserve", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
